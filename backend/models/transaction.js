...
exports.addDataTransaction = async (
  ...
  product_id,
  price_id,
  bill_number_engineer,
  code,
  consumer_type,
  qty,
  subtotal
) => {
  const sql = `INSERT INTO transaction (store_id_transaction, distributor_id_transaction, product_id_transaction, price_id_transaction, bill_number_engineer_transaction, code, consumer_type, qty, subtotal) values (${store_id}, ${distributor_id}, ${product_id}, ${price_id}, '${bill_number_engineer}', '${code}', '${consumer_type}', ${qty}, ${subtotal})`;
  const result = await poolConnection.query(sql);
  return result[0];
};
