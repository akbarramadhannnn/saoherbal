...
import { ApiUpdateListStore, ApiDetailListStore } from "../../../api/store";
...
const Update = props => {
  ...
  useEffect(() => {
    ...
    ApiDetailListStore(id).then(response => {
      if (response) {
        if (response.status === 200) {
          setName(response.result.name);
          setTlp(response.result.no_tlp);
          setEmail(response.result.email);
          setProvinsi(response.result.store_prov_id);
          setKabupaten(response.result.store_kab_id);
  ...
  const handleSave = useCallback(() => {
    ...
    ApiUpdateListStore(id, payload).then(response => {
      if (response) {
